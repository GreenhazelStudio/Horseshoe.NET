...
namespace Horseshoe.NET.DataAccess.OracleDb.Meta
{
    public static class MetaSearch
    {
        ...
        public static class Schemas
        {
            ...
            public static IEnumerable<OraSchema> List(OraConnectionInfo connectionInfo, Predicate<OraSchema> filter = null, int? timeout = null)
            {
                ...
                using (var conn = OracleUtil.LaunchConnection(connectionInfo: connectionInfo))
                {
                    return List(conn, server, filter: filter, timeout: timeout);
                }
            }
            ...
            {
                using (var conn = OracleUtil.LaunchConnection(connectionInfo: new OraConnectionInfo { Server = server ?? throw new UtilityException("Server cannot be null"), Credentials = credentials }))
                {
                    return List(conn, server, filter: filter, timeout: timeout);
                }
            }
            ...
            {
                conn = OracleUtil.LaunchConnection(connectionInfo: new OraConnectionInfo { Server = server ?? throw new UtilityException("Server cannot be null"), Credentials = credentials });
                return List(conn, server, filter: filter, timeout: timeout);
            }
            ...
            public static OraSchema Lookup(OraConnectionInfo connectionInfo, string name, bool ignoreCase = false, int? timeout = null)
            {
                var server = connectionInfo.Server ?? new OraServer(connectionInfo.DataSource ?? throw new UtilityException("Server and DataSource cannot both be null"));
                return Lookup
                (
            ...
            {
                using (var conn = OracleUtil.LaunchConnection(new OraConnectionInfo { Server = server ?? throw new UtilityException("Server cannot be null"), Credentials = credentials }))
                {
                    return Lookup
                    (
                        conn,
        ...
        public static class Objects
        {
            ...
            {
                using (var conn = OracleUtil.LaunchConnection(connectionInfo: new OraConnectionInfo { Server = (schema ?? throw new UtilityException("Schema cannot be null")).Server ?? throw new UtilityException("No Server ancestor exists for the supplied Schema"), Credentials = credentials }))
                {
                    return List(conn, schema, objectType: objectType, filter: filter, timeout: timeout);
                }
            }
            ...
            {
                using (var conn = OracleUtil.LaunchConnection(connectionInfo: new OraConnectionInfo { Server = schema.Server ?? throw new UtilityException("No Server ancestor exists for the supplied Schema"), Credentials = credentials }))
                {
                    return Lookup(conn, schema, name, ignoreCase: ignoreCase, timeout: timeout);
                }
            }
        ...
        public static class Columns
        {
            ...
            {
                using (var conn = OracleUtil.LaunchConnection(connectionInfo: new OraConnectionInfo { Server = tableOrView.Server ?? throw new UtilityException("No Server ancestor exists for the supplied " + (tableOrView?.ObjectType.ToString() ?? "Object")), Credentials = credentials }))
                {
            ...
            public static IEnumerable<OraColumn> List(OracleConnection conn, OraObject tableOrView, Predicate<OraColumn> filter = null, int? timeout = null)
            {
                ...
                (
                    ...
                    )
                    {
                        ...
                        Length = ObjectUtil.ZapInt(reader["length"]),
                        Precision = ObjectUtil.ZapNInt(reader["precision"]),
                        Scale = ObjectUtil.ZapNInt(reader["scale"]),
                        IsNullable = ObjectUtil.ZapBoolean(reader["isNullable"]),
                        ColumnID = Convert.ToInt32(reader["columnID"]),
                        DefaultValue = ObjectUtil.ZapString(reader["defaultValue"])
                    },
                    timeout: timeout
                );
            ...
            {
                using (var conn = OracleUtil.LaunchConnection(connectionInfo: new OraConnectionInfo { Server = column.Server ?? throw new UtilityException("No Server ancestor exists for the supplied Column"), Credentials = credentials }))
                {
                    conn.Open();
