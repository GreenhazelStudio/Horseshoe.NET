...
namespace Horseshoe.NET.Objects
{
    ...
    {
        public static object Object(object obj, TextCleanRules textCleanRules = null, Func<object, bool> evaluatesToNull = null)
        {
            if (obj is null || obj is DBNull) return null;
            if (evaluatesToNull?.Invoke(obj) ?? false) return null;
            if (obj is string stringValue) return _String(stringValue, textCleanRules);
            if (obj is StringValues stringValuesValue)
            {
                switch (stringValuesValue.Count)
                {
                    case 0: return null;
                    case 1: return _String(stringValuesValue.Single(), textCleanRules);
                    default: return string.Join(", ", stringValuesValue.Select(s => _String(s, textCleanRules)));
                }
            }
            return obj;
        }

        public static string String(object obj, TextCleanRules textCleanRules = null, Func<object, bool> evaluatesToNull = null)
        {
            if ((obj = Object(obj, evaluatesToNull: evaluatesToNull)) is null) return null;
            if (obj is string stringValue)
            {
                return _String(stringValue, textCleanRules);
            }
        ...
        static string _String(string stringValue, TextCleanRules textCleanRules)
        {
            stringValue = stringValue.Trim();
            if (stringValue.Length > 0)
            {
                stringValue = TextClean.CleanString(stringValue, rules: textCleanRules);
                stringValue = stringValue.Trim();
            }
            if (stringValue.Length == 0) return null;
            return stringValue;
        ...
        public static T? NEnum<T>(object obj, bool ignoreCase = false, bool suppressErrors = false, Func<object, bool> evaluatesToNull = null) where T : struct
        {
            ...
            if (obj is string stringValue)
            {
                stringValue = TextClean.CleanString(stringValue, rules: new TextCleanRules { Mode = TextCleanMode.RemoveWhitespace });
