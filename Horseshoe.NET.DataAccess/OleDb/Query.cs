...
namespace Horseshoe.NET.DataAccess.OleDb
{
    public static class Query
    {
        ...
        public static class SQL
        {
            ...
            {
                using (var command = OleDbUtil.BuildCommand(conn, CommandType.Text, statement, timeout: timeout))
                {
            ...
            {
                var command = OleDbUtil.BuildCommand(conn, CommandType.Text, statement, timeout: timeout);
                var reader = keepOpen
        ...
        public static class StoredProcedure
        {
            ...
            )
            {
                ...
                {
                    if (readerParser != null) throw new UtilityException("This method accepts a maximum of one parser");
                    using (var command = OleDbUtil.BuildCommand(conn, CommandType.StoredProcedure, procedureName, parameters: parameters, timeout: timeout))
                    {
            ...
            {
                var command = OleDbUtil.BuildCommand(conn, CommandType.StoredProcedure, procedureName, parameters: parameters, timeout: timeout);
                var reader = keepOpen
                    ? command.ExecuteReader(CommandBehavior.Default)
                    : command.ExecuteReader(CommandBehavior.CloseConnection);
                return reader;
        ...
        public static class Function
        {
            ...
            )
            {
                ...
                using (var command = OleDbUtil.BuildCommand(conn, CommandType.Text, statement, timeout: timeout))  // not including parameters here due to already embedded in the SQL statement
                {
                    var obj = ObjectUtil.Zap(command.ExecuteScalar());
                    if (obj is string stringValue)
                    {
            ...
            )
            {
                ...
                var command = OleDbUtil.BuildCommand(conn, CommandType.Text, statement, timeout: timeout);  // not including parameters here due to already embedded in the SQL statement
                var reader = keepOpen
                    ? command.ExecuteReader(CommandBehavior.Default)
                    : command.ExecuteReader(CommandBehavior.CloseConnection);
                return reader;
