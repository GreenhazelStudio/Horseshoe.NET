...
namespace Horseshoe.NET.DataAccess.Sql.Meta
{
    public static class MetaSearch
    {
        ...
        public static class Databases
        {
            ...
            public static IEnumerable<Db> List(SqlConnectionInfo connectionInfo, Predicate<Db> filter = null, int? timeout = null)
            {
                ...
                using (var conn = SqlUtil.LaunchConnection(connectionInfo: connectionInfo))
                {
                    return List(conn, server, filter: filter, timeout: timeout);
                }
            }
            ...
            {
                using (var conn = SqlUtil.LaunchConnection(connectionInfo: new SqlConnectionInfo { Server = server ?? throw new UtilityException("Server cannot be null"), Credentials = credentials }))
                {
                    return List(conn, server, filter: filter, timeout: timeout);
                }
            }
            ...
            {
                conn = SqlUtil.LaunchConnection(connectionInfo: new SqlConnectionInfo { Server = server ?? throw new UtilityException("Server cannot be null"), Credentials = credentials });
                return List(conn, server, filter: filter, timeout: timeout);
            }
            ...
            public static Db Lookup(SqlConnectionInfo connectionInfo, string name, bool ignoreCase = false, int? timeout = null)
            {
                var server = connectionInfo.Server ?? new DbServer(connectionInfo.DataSource ?? throw new UtilityException("Server and DataSource cannot both be null"));
                return Lookup
                (
            ...
            {
                using (var conn = SqlUtil.LaunchConnection(new SqlConnectionInfo { Server = server ?? throw new UtilityException("Server cannot be null"), Credentials = credentials }))
                {
                    return Lookup
                    (
                        conn,
        ...
        public static class Objects
        {
            ...
            {
                using (var conn = SqlUtil.LaunchConnection(connectionInfo: new SqlConnectionInfo { Server = (database ?? throw new UtilityException("Database cannot be null")).Server ?? throw new UtilityException("No Server ancestor exists for the supplied Database"), Credentials = credentials }))
                {
                    return List(conn, database, objectType: objectType, filter: filter, timeout: timeout);
                }
            }
            ...
            {
                using (var conn = SqlUtil.LaunchConnection(connectionInfo: new SqlConnectionInfo { Server = database.Server ?? throw new UtilityException("No Server ancestor exists for the supplied Database"), Credentials = credentials }))
                {
                    return Lookup(conn, database, name, ignoreCase: ignoreCase, timeout: timeout);
                }
            }
        ...
        public static class Columns
        {
            ...
            {
                using (var conn = SqlUtil.LaunchConnection(connectionInfo: new SqlConnectionInfo { Server = tableOrView.Server ?? throw new UtilityException("No Server ancestor exists for the supplied " + (tableOrView?.ObjectType.ToString() ?? "Object")), Credentials = credentials }))
                {
            ...
            {
                using (var conn = SqlUtil.LaunchConnection(connectionInfo: new SqlConnectionInfo { Server = obj.Server ?? throw new UtilityException("No Server ancestor exists for the supplied " + (obj?.ObjectType.ToString() ?? "Object")), Credentials = credentials }))
                {
            ...
            public static IEnumerable<DbColumn> SearchByValue(SqlConnection conn, DbObject obj, object value, Predicate<DbColumn> filter = null, int? timeout = null)
            {
            ...
            private static bool DoesColumnContainValue(SqlConnection conn, DbColumn column, object value, int? timeout)
            {
                ...
                {
                    statement += column + " = " + DataUtil.Sqlize(value);
                }
            ...
            {
                using (var conn = SqlUtil.LaunchConnection(connectionInfo: new SqlConnectionInfo { Server = column.Server ?? throw new UtilityException("No Server ancestor exists for the supplied Column"), Credentials = credentials }))
                {
                    conn.Open();
